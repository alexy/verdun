...
pub fn public_snapshot_value_as_crawler_snapshot(
    value: serde_json::Value,
    path: &PathBuf,
) -> Result<CrawlerSnapshot> {
    Ok(export_payload_from_public_snapshot(value, path)?.normalized_snapshot())
}

pub fn load_newsletter_export_payload(
    snapshot: Option<&PathBuf>,
    input: &PathBuf,
    source_runs: &PathBuf,
    ...
    if let Some(snapshot) = snapshot {
        ...
        return export_payload_from_public_snapshot(value, snapshot);
    }
    ...
    let source_runs: Vec<SourceRun> = if source_runs.exists() {
        serde_json::from_slice(
            &fs::read(source_runs).with_context(|| format!("reading {}", source_runs.display()))?,
        )?
    } else {
        Vec::new()
    };
...
fn export_payload_from_public_snapshot(
    value: serde_json::Value,
    path: &PathBuf,
) -> Result<ExportPayload> {
