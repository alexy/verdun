...
fn export_sql(
    ...
    let (items, source_runs, query_plans) = load_export_payload(snapshot, input, source_runs)?;
...
fn load_export_payload(
    ...
    input: PathBuf,
    source_runs: PathBuf,
) -> Result<(Vec<NewsItem>, Vec<SourceRun>, Vec<ProjectQueryPlan>)> {
    if let Some(snapshot) = snapshot {
        let snapshot: PublicSnapshot = serde_json::from_slice(
            &fs::read(&snapshot).with_context(|| format!("reading {}", snapshot.display()))?,
        )?;
        return Ok((snapshot.items, snapshot.source_runs, snapshot.query_plans));
    }
