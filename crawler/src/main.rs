...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        CommandKind::ExportSql {
            ...
            snapshot,
            target,
        } => export_sql(snapshot, input, source_runs, out, target),
...
fn export_sql(
    ...
    let sql = match target {
        ...
        SqlExportTarget::Generic => generic_export_sql(&payload)?,
    };
...
fn generic_export_sql(payload: &ExportPayload) -> Result<String> {
    let mut sql =
    ...
    sql.push_str("values ('garbage', 'Garbage', '/rbage/', ");
    sql.push_str(&sql_string(&payload.theme));
    sql.push_str(", now())\n");
    sql.push_str("on conflict (id) do update set\n");
    ...
    for item in &payload.items {
        ...
        sql.push_str(&item.score.to_string());
        sql.push_str(", 'active', ");
        sql.push_str(&sql_string(&item_dedupe_key(item)));
        sql.push_str(", ");
        sql.push_str(&sql_string(&serde_json::to_string(&provenance_json)?));
        sql.push_str("::jsonb, ");
        sql.push_str(&sql_string(&serde_json::to_string(&normalized_json)?));
        sql.push_str("::jsonb, ");
        sql.push_str(&sql_string(&serde_json::to_string(&item.raw_json)?));
        sql.push_str("::jsonb, now())\n");
