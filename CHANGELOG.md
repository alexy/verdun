...
- The Rust crawler now has a Garbage instance module for snapshot/domain structs, normalization, dedupe, project-count aggregation, source-run construction, manual-source freshness wording, watchlist seed item construction, provenance metadata, parsed source records, item constructors, query/focus planning, and the HN live fetch adapter, but Lobste.rs/dev.to/feed/manual fetch adapters, parsing, and matching still live in `crawler/src/main.rs`.
