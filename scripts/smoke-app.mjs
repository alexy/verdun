...
try {
  ...
  const firstCredoHref = await firstCredoLink.getAttribute('href')
  if (!firstCredoHref?.startsWith('#ontology-')) throw new Error('credo fit link does not target the maintained ontology')
  await firstCredoLink.locator('small').getByText(/\w+/).waitFor()
