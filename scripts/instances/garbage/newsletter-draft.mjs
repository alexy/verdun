...
export async function runNewsletterDraftCli(args, env = process.env) {
  const cli = parseCliArgs(args)
  const ulyssesMode = cli.flags.has('--ulysses')
  const requireUpvotes = cli.flags.has('--require-upvotes') || env.NEWSLETTER_REQUIRE_UPVOTES === 'true'
  const requireReady = cli.flags.has('--require-ready') || env.NEWSLETTER_REQUIRE_READY === 'true'
  const ulyssesImportDir = cli.values.get('--ulysses-import-dir') ?? env.ULYSSES_IMPORT_DIR
  const editorialStateInput = cli.values.get('--editorial-state') ?? env.VERDUN_LOCAL_STATE_FILE
  if (editorialStateInput) env.VERDUN_LOCAL_STATE_FILE = editorialStateInput
  const positional = cli.positional
  const input = positional[0] ?? env.NEWSLETTER_SNAPSHOT_FILE ?? 'public/data/newsletter-snapshot.json'
  ...
  const out = positional[1] ?? env.NEWSLETTER_DRAFT_OUT ?? defaultDraftPath(draft, snapshot, ulyssesMode)
