...
The bundled Garbage API workbench adapter and view-model smoke import the parent package's `apps/garbage/src/workbench.ts` projection, so API fallback/workbench routes and view-model coverage use the external Garbage package's newsletter-to-workbench mapping while the resident Verdun frontend projection remains as compatibility until the UI boundary moves.
