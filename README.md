...
The bundled Garbage API workbench adapter imports the parent package's `apps/garbage/src/workbench.ts` projection, so API fallback/workbench routes use the external Garbage package's newsletter-to-workbench mapping while the resident Verdun frontend projection remains as compatibility until the UI boundary moves.
