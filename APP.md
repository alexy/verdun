...
- The parent Garbage repo now exposes `@garbage/instance` package commands as the temporary stable command surface while Garbage implementation moves out of Verdun. Publishing entrypoints live in the parent package and still import bundled Verdun newsletter modules; operational compatibility smokes still delegate through the external instance manifest into declared Verdun scripts.
...
- Replace parent Garbage package imports of bundled Verdun newsletter modules with package-owned implementations one command group at a time.
