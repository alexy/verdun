...
- The bundled Garbage API workbench adapter imports `apps/garbage/src/workbench.ts`, so generic workbench fallback routes now consume the parent package's Garbage projection while the resident Verdun projection remains only for frontend/view-model compatibility.
