...
- Source-health coverage gaps identify watched projects without live/manual source matches, show crawler query hints for the first gaps, can be saved directly as this-week focus requests, and include the same actionable gap signal in local Markdown drafts.
- `verdun-crawler queries` and `collect` read exported/local editorial state when available and attach matching saved focus terms to project query plans.
