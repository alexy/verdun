...
- The crawler SDK now exposes reusable JSON disk cache, artifact writer, run-manifest, and freshness helpers through `verdun_crawler::sdk`: `JsonDiskCache`, `CacheRead`, `write_pretty_json`, `write_text`, `CrawlerRunManifest`, `CrawlerOutputPaths`, `SourceRunSummary`, `FreshnessStatus`, `FreshnessPolicy`, and `FreshnessAssessment`. Verdun's own crawler runtime uses these helpers for collection/export output writes and can write shared lifecycle metadata with `collect --run-manifest-out`; `collect --fresh-after-hours` records `snapshot_freshness`; tests cover deterministic cache paths, source-run summaries, freshness status, and parent-directory creation.
