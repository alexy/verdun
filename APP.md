...
- Generic SQL export adapts Garbage newsletter snapshots into reusable `CrawlerSnapshot`, `NormalizedRecord`, and `NormalizedCollectionPlan` structs before writing Verdun's generic tables.
...
- External DB loader SQL can export from the cohesive public snapshot either to the legacy Garbage/newsletter tables or to the reusable generic Verdun tables with `export-sql --target generic`, while preserving the snapshot collection timestamp.
