...
- The parent Garbage repo now exposes `@garbage/instance` package commands that delegate through the external instance manifest into declared Verdun scripts; this is the temporary stable command surface while Garbage implementation moves out of Verdun.
...
- Replace parent Garbage package command delegation with package-owned implementations one command group at a time.
