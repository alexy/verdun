...
- The crawler SDK now exposes reusable HTTP fetch metadata, JSON disk cache, JSON cache audit, artifact writer, run-manifest, and freshness helpers through `verdun_crawler::sdk`: `HttpFetchMetadata`, `HttpFetch<T>`, text/JSON/status-preserving fetch helpers, `JsonDiskCache`, `CacheRead`, `CacheNamespacePolicy`, `JsonCacheNamespaceAudit`, `JsonCacheAudit`, `audit_json_cache`, `is_cache_metadata_path`, `write_pretty_json`, `write_text`, `CrawlerRunManifest`, `CrawlerOutputPaths`, `SourceRunSummary`, `FreshnessStatus`, `FreshnessPolicy`, and `FreshnessAssessment`. Verdun's own crawler runtime uses these helpers for collection/export output writes and can write shared lifecycle metadata with `collect --run-manifest-out`; `collect --fresh-after-hours` records `snapshot_freshness`; tests cover deterministic cache paths, cache audit summaries, HTTP fetch metadata, source-run summaries, freshness status, and parent-directory creation.
...
- Generic database apply/deploy helpers are exposed to external apps through `scripts/public/workbench-apply-sql.mjs` and `scripts/public/deploy-workbench-database.mjs`; `deploy-workbench-database --handoff-out <path>` writes a redacted reload handoff tying together source snapshot, SQL load, instance/base path, migrations, apply/preflight mode, database-env presence, and deployed-check command.
