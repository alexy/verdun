...
- The bundled Garbage API workbench adapter and view-model smoke import `apps/garbage/src/workbench.ts`, so generic workbench fallback routes and view-model coverage now consume the parent package's Garbage projection while the resident Verdun projection remains only for compatibility until the UI boundary moves.
